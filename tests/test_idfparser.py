...
def test_sample_position_is_at_origin():
    ...
    test_sample = {"name": "TEST_SAMPLE", "position": sample_pos}
    fake_idf_file = create_fake_idf_file(sample=test_sample, source_name="the_source")
    parser = IDFParser(fake_idf_file)
    np.testing.assert_allclose(parser.get_sample_position(), np.array([0.0, 0.0, 0.0]))
    # Check that source is offset from IDF into nexus frame
    np.testing.assert_allclose(
        parser.get_source_position(),
        np.array(list(map(operator.sub, source_pos, sample_pos))),
    )
    fake_idf_file.close()
...
def test_get_length_units_returns_same_length_unit_string_as_specified_in_IDF():
    test_defaults = {"length_units": "cubits", "angle_units": "deg"}
...
def test_get_angle_units_returns_deg_when_specified_as_deg_in_IDF():
    test_defaults = {"length_units": "Yojana", "angle_units": "deg"}
    fake_idf_file = create_fake_idf_file(defaults=test_defaults)
    parser = IDFParser(fake_idf_file)
...
def test_get_angle_unit_on_other_than_rad_or_deg_fails():
    test_defaults = {"length_units": "Ald", "angle_units": "Furman"}
    fake_idf_file = create_fake_idf_file(defaults=test_defaults)
...
def test_get_structured_detectors_returns_detector_details():
    x_pos = np.hstack(
        (
            ...
            np.linspace(-1.5, 1.5, 4),
            np.linspace(-0.3, 0.3, 4),
        )
    )
    y_pos = np.hstack(np.array([[1.0] * 4, [0.0] * 4, [-1.0] * 4]))
    z_pos = np.array([0.0] * 12)
    input_vertices = np.hstack(
        (np.expand_dims(x_pos, 1), np.expand_dims(y_pos, 1), np.expand_dims(z_pos, 1))
    )
    expected_output = {
        "type_name": "fan",
        "orientation": None,
        "Y_id_step": 2,
        "location": np.array([0.0, 0.01, 9.23]),
        "id_start": 0,
        "X_id_step": 1,
        "name": "TEST_STRUCT_DET",
    }
    fake_idf_file = create_fake_idf_file(
        structured_detector={
...
def test_get_rectangular_detectors_returns_expected_pixel_offsets():
    ...
    parser = IDFParser(fake_idf_file)
    output_detectors = list(parser.get_rectangular_detectors())
    expected_x_offsets = np.linspace(
        detector["xstart"],
        detector["xstart"] + (detector["xstep"] * (detector["xpixels"] - 1)),
        detector["xpixels"],
    )
    expected_y_offsets = np.linspace(
        detector["ystart"],
...
def test_get_rectangular_detectors_returns_expected_ids():
    ...
    expected_ids = np.array(
        [
            [
                detector["idstart"],
                detector["idstart"] + detector["idstep"],
                detector["idstart"] + (detector["idstep"] * 2),
            ],
            [
                detector["idstart"] + 1,
                detector["idstart"] + detector["idstep"] + 1,
                detector["idstart"] + (detector["idstep"] * 2) + 1,
            ],
            [
                detector["idstart"] + 2,
                detector["idstart"] + detector["idstep"] + 2,
                detector["idstart"] + (detector["idstep"] * 2) + 2,
            ],
        ]
    ).astype(int)
...
def test_rectangular_detectors_have_default_id_parameters():
    ...
    expected_ids = np.array(
        [
            [
                default_idstart,
                default_idstart + default_idstep,
                default_idstart + (default_idstep * 2),
            ],
            [
                default_idstart + 1,
                default_idstart + default_idstep + 1,
                default_idstart + (default_idstep * 2) + 1,
            ],
            [
                default_idstart + 2,
                default_idstart + default_idstep + 2,
                default_idstart + (default_idstep * 2) + 2,
            ],
        ]
    ).astype(int)
    assert np.array_equal(expected_ids, output_detectors[0]["idlist"])
...
def test_nested_rotations_on_rectangular_detector():
    ...
    fake_idf_file = create_fake_idf_file(
        rectangular_detector=detector, custom_location=location
    )
    parser = IDFParser(fake_idf_file)
