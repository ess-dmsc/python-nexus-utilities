...
def __write_rectangular_detector_component(
    ...
    if "idstart" in detector.keys() and "idstep" in detector.keys():
        fake_idf_file.write(
            '  <component type="detector-bank" idstart="'
            + str(detector["idstart"])
            + '" idfillbyfirst="y" idstep="'
            + str(detector["idstep"])
            + '" idstepbyrow="1">\n'
            f"  {location}\n"
            "  </component>\n"
...
def __write_idlist(fake_idf_file):
    fake_idf_file.write(
        '<idlist idname="test-idlist">\n' '  <id start="1" end="3" />\n' "</idlist>\n"
    )


def __write_detector_panel(fake_idf_file):
    fake_idf_file.write(
        '<component type="small-detector" idlist="test-idlist" name="panel01">\n'
...
def __create_pixel_shape(pixel_shape):
    if pixel_shape["shape"] == "cylinder":
        return (
            ...
            '    <axis x="'
            + str(pixel_shape["axis"][0])
            + '" y="'
            + str(pixel_shape["axis"][1])
            + '" z="'
            + str(pixel_shape["axis"][2])
            + '" />\n'
            '    <radius val="' + str(pixel_shape["radius"]) + '" />\n'
    ...
    if pixel_shape["shape"] == "cuboid":
        return (
            ...
            + str(pixel_shape["x_pixel_size"] / 2.0)
            + '" y="'
            + str(pixel_shape["y_pixel_size"] / -2.0)
            + '" z="'
            + str(0.0)
            + '"  />'
...
def __write_structured_detector(fake_idf_file, structured_detector):
    fake_idf_file.write(
        '<component name ="'
        + structured_detector["name"]
        + '" type="'
        + structured_detector["type"]
        + '" idstart="0" idfillbyfirst="x" idstepbyrow="1" idstep="2">\n'
    ...
    for vertex in structured_detector["vertices"]:
        fake_idf_file.write(
            '  <vertex x="' + str(vertex[0]) + '" y="' + str(vertex[1]) + '" />\n'
        )
...
def __write_monitors(fake_idf_file, monitor_name):
    ...
    fake_idf_file.write(
        '<type name="monitors"><component type="monitor-tbd">'
        '<location z="7.217" name="' + monitor_name + '"/></component></type>\n'
    )
    fake_idf_file.write(
        '<type name="monitor-tbd" is="monitor"><cylinder id="some-shape"><centre-of-bottom-base '
...
def __write_defaults(defaults, fake_idf_file):
    fake_idf_file.write(
        '<defaults><length unit="'
        + defaults["length_units"]
        + '"/><angle unit="'
        + defaults["angle_units"]
        + '"/><reference-frame><along-beam axis="z"/>'
...
def __write_sample(fake_idf_file, sample):
    fake_idf_file.write(
        ...
        + sample["name"]
        + '"><location x="'
        + str(sample["position"][0])
        + '" y="'
        + str(sample["position"][1])
        + '" z="'
        + str(sample["position"][2])
        + '"/></component>'
    )
...
def __write_source(fake_idf_file, source_name):
    ...
    fake_idf_file.write(
        '<component type="' + source_name + '"><location z="-40.0"/></component>\n'
    )
