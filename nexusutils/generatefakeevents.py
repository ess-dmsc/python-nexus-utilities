...
def generate_fake_events(
    nexus_entry: h5py._hl.group.Group,
    events_per_pulse,
    number_of_pulses,
    pulse_freq_hz=10.0,
    tof_min_ns=0,
    ...
    for instrument in __get_nodes_of_class(nexus_entry, "NXinstrument"):
        for detector in __get_nodes_of_class(instrument, "NXdetector"):
            ...
            event_time_offset = np.random.randint(
                size=(number_of_events_total),
                dtype=np.uint64,
                low=tof_min_ns,
                high=tof_max_ns,
            )
            event_group = detector.create_group("event_data")
            event_group.attrs.create(
                "NX_class", np.array("NXevent_data").astype("|S12")
            )
            create_dataset(
                nexus_entry,
                event_group,
            ...
            nexus_entry[
                "event_data_{}".format(detector.name.split("/")[-1])
            ] = event_group
    return all_ids
