...
class LOKIGenerator(object):
    """ Generates an Instrument Definition File (IDF) for the LOKI instrument """

    def __init__(self, sectors, inner_radius, outer_radius, phi_slice, y_step, theta_tilt, output_file):
        """ Initialises the LOKIGenerator object
        Args:
            sectors: Number of sectors per detector bank
            inner_radius: Sector inner radius in metres
            outer_radius: Sector outer radius in metres 
            phi_slice: Number of phi slices per sector
            y_step: Step length in y in metres
            theta_tilt: Tilt angle which may not necessarily be on xy plane
            output_file: File name where XML IDF will be stored
        """
        # Define the segmentation of the
        # detector surface
        self.xSeg = phi_slice
        self.ySeg = (outer_radius - inner_radius) / y_step

        self.ySeg = int(math.ceil(self.ySeg))

        # metres
        self.ymin = inner_radius
        self.ymax = outer_radius
    ...
    def _calc_vertices(self):
        ...
        for y in range(self.ySeg + 1):
            theta = -angleInc * (self.xSeg / 2)
            for x in range(self.xSeg + 1):
                if theta == 0:
                    x_ = 0
                else:
    ...
    @staticmethod
    def _write_idf_samples_and_sources(f):
        ...
        for i in range(num_sources):
            ...
            f.write("\t\t<centre-of-bottom-base x=\"" + str(xyval * val[c1]) + "\" y=\"" + str(
                xyval * val[c2]) + "\" z=\"" + str(zval) + "\" />\n")
            f.write("\t\t<axis x=\"" + str(0.000555556 * val[c1]) + "\" y=\"" + str(
                0.000555556 * val[c2]) + "\" z=\"0.333333\" />\n")
        ...
        for i in range(num_samples):
            ...
            f.write("\t<cylinder id=\"dummy\" >\n")
            f.write("\t\t<centre-of-bottom-base x=\"" + str(0.005 * c1) + "\" y=\"" + str(0.005 * c2) + "\" z=\"" + str(
                0.0025 * c3) + "\" />\n")
    ...
    def _write_idf_detector_banks(self, f):
        ...
        f.write(
            "<component name =\"MonNDtype-0\" type=\"fan\" idstart=\"0\" idfillbyfirst=\"x\" idstepbyrow=\"1\" idstep=\"100\">\n")
        ...
        for i in range(self.numPieces - 1):
            f.write("<component name =\"MonNDtype-" + str(i + 1) + "\" type=\"fan\" idstart=\"" + str(
                id) + "\" idfillbyfirst=\"x\" idstepbyrow=\"1\" idstep=\"100\">\n")
            f.write("<!--\n")
            f.write("Detector Panel " + str(i + 2) + " with offset x=0.01*sin(" + str(-angle) + ") y=0.01*cos(" + str(
                angle) + ")\n ")
            f.write("the negative angle in x produces the correct sign for the x offset-->\n")
            if angle <= 180:
                f.write("\t<location x=\"" + str(0.01 * math.sin(self.piDiv * -angle)) + "\" y=\"" + str(
                    0.01 * math.cos(self.piDiv * angle)) + "\" z=\"9.23\" rot=\"" + str(
                    angle) + "\" axis-x=\"0.0\" axis-y=\"0.0\" axis-z=\"1.0\" />\n")
            else:
                f.write("\t<location x=\"" + str(0.01 * math.sin(self.piDiv * -angle)) + "\" y=\"" + str(
                    0.01 * math.cos(self.piDiv * angle)) + "\" z=\"9.23\" rot=\"" + str(
                    360 - angle) + "\" axis-x=\"0.0\" axis-y=\"0.0\" axis-z=\"-1.0\" />\n")
            angle += 45.0
            f.write("</component>\n\n")
            id += 1000

    def _write_idf_vertices(self, f):
        """
        ...
        Args:
            f: File handle
        """
        f.write("<type name=\"fan\" is=\"StructuredDetector\" xpixels=\"" + str(self.xSeg) + "\" ypixels=\"" + str(
            self.ySeg) + "\" type=\"pixel\">\n")
        f.write("<!--\n")
        ...
        for i in range(int(self.printArr.size / 3)):
    ...
    def _write_xml_idf(self):
        """
        Writes the IDF file using vertices and information about the detector
        """
        with open(self.outputFile, "w") as f:
            # write xml header
            self._write_idf_header(f)

            # write defaults
            self._write_idf_defaults(f)

            # write samples and sources
            self._write_idf_samples_and_sources(f)

            # write components
            self._write_idf_detector_banks(f)

            # write Detector type and vertices
            self._write_idf_vertices(f)

    def generate_loki(self):
        """Generates Loki IDF"""
        self._calc_vertices()
        self._write_xml_idf()
